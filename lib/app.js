...
var GoDiApp = App.extend(function(self) {
    ...
    self.is_registered = function() {
        return (typeof self.contact.extra.is_registered === 'undefined' || self.contact.extra.is_registered === "false");
    };
    ...
    self.init = function() {
        ...
        return self.im.contacts.for_user()
            ...
            .then(function() {
                return self.im.on('session:close', function(e) {
                    if (e.user_terminated && self.is_delivery_class("ussd")) {
                        var contact = self.contact.extra;

                        //Get if registered and that sms hasn't already been sent
                        if (self.is_registered() && !self.is(contact.register_sms_sent)) {

                            //Check ward
                            if (_.isUndefined(contact.ward)) {
